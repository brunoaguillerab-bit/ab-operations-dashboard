    ...
    <div className="flex-1 overflow-y-auto p-6">
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="rounded-xl border border-white/10 bg-amber-500/5 p-4 backdrop-blur-sm">
            <p className="text-sm text-[#A1A1AA]">Total de Campanhas</p>
            <p className="text-3xl font-bold text-white mt-2">{googleCampaigns.length}</p>
          </div>
          <div className="rounded-xl border border-white/10 bg-green-500/5 p-4 backdrop-blur-sm">
            ...
            <p className="text-3xl font-bold text-green-400 mt-2">
              {googleCampaigns.filter(d => d.status === 'Em andamento').length}
            </p>
          </div>
          <div className="rounded-xl border border-white/10 bg-amber-500/5 p-4 backdrop-blur-sm">
            <p className="text-sm text-[#A1A1AA]">Saldo Total</p>
            <p className="text-3xl font-bold text-amber-400 mt-2">
              R$ {googleCampaigns.reduce((sum, d) => sum + (d.saldoContaGoogleAds || 0), 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
            </p>
          </div>
        </div>
        ...
        {googleCampaigns.length === 0 ? (
          <div className="rounded-xl border border-white/10 bg-white/5 p-8 text-center backdrop-blur-sm">
            ...
            <p className="text-[#A1A1AA]">Nenhuma campanha Google Ads cadastrada</p>
          </div>
        ) : (
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  <div className="flex-1">
                    <h3 className="font-semibold text-white flex items-center gap-2">
                      ...
                      {campaign.urlGoogleAds && (
                        <a
                          href={campaign.urlGoogleAds}
                          target="_blank"
                  ...
                  <div className="flex flex-col md:flex-row gap-4 md:items-center">
                    <div className="text-right">
                      ...
                      <p className="text-lg font-semibold text-white">
                        R$ {(campaign.saldoContaGoogleAds || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                      </p>
                    </div>
